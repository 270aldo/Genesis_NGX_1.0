...
from core.intent_analyzer import Intent, intent_analyzer
...
class IntentAnalyzerAdapter:
    ...
    def __init__(self, use_optimized: bool = False):
        ...
        logger.info(
            f"Adaptador de analizador de intenciones inicializado (use_optimized={use_optimized})"
        )
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(
                "Adaptador de analizador de intenciones inicializado correctamente"
            )
            return True

        except Exception as e:
            logger.error(
                f"Error al inicializar adaptador de analizador de intenciones: {e}"
            )
            self.stats["errors"] += 1
            return False
    ...
    def set_use_optimized(self, use_optimized: bool) -> None:
        ...
        self.use_optimized = use_optimized
        logger.info(
            f"Cambiado analizador a: {'optimizado' if use_optimized else 'original'}"
        )
    ...
    async def analyze_intent(
        ...
        try:
            ...
            if self.use_optimized:
                ...
                intents = await self._original_analyzer.analyze_content(
                    user_query=user_query,
                    conversation_id=conversation_id,
                    user_id=user_id,
                    context=context,
    ...
    async def analyze_intents_with_embeddings(
        ...
        try:
            ...
            if self.use_optimized:
                ...
                intents = await self._optimized_analyzer.analyze_query(
                    user_query=user_query, conversation_id=conversation_id
                )
            ...
            logger.error(
                f"Error en adaptador de analizador de intenciones (embeddings): {e}"
            )
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        if self.stats["total_queries"] > 0:
            stats["avg_processing_time"] = (
                self.stats["processing_time"] / self.stats["total_queries"]
            )
