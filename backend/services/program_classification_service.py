...
class ProgramClassificationService:
    ...
    def __init__(
        self, vertex_ai_client: Optional[VertexAIClient] = None, use_cache: bool = True
    ):
        """
        Inicializa el servicio de clasificación de programas.

        Args:
            vertex_ai_client: Cliente de Gemini para clasificación basada en LLM
    ...
    async def classify_program_type(
        ...
        if use_llm and self.vertex_ai_client:
            try:
        ...
        elif use_llm and not self.vertex_ai_client:
            self.logger.warning(
                "VertexAIClient no disponible para clasificación avanzada de tipo de programa"
            )
    ...
    async def _classify_with_llm(self, context: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            if hasattr(self.vertex_ai_client, "generate_structured_output"):
                self.logger.debug(
                    "Usando generate_structured_output para clasificar tipo de programa"
                )
                classification = await self.vertex_ai_client.generate_structured_output(
                    prompt
                )
            else:
