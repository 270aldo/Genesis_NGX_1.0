...
class WearableDataNormalizer:
    ...
    def __init__(self):
        # No need for device_handlers map as we handle normalization directly
        # in the normalize_* methods with device type checks
        pass
    ...
    def normalize_recovery_data(
        ...
        elif device == WearableDevice.GARMIN:
            return self._normalize_garmin_recovery(raw_data)
        else:
    ...
    def normalize_sleep_data(
        ...
        if device == WearableDevice.WHOOP:
            ...
            return self._normalize_garmin_sleep(raw_data)
        else:
    ...
    def normalize_workout_data(
        ...
        if device == WearableDevice.WHOOP:
            ...
            return self._normalize_garmin_workout(raw_data)
        else:
    ...
    def _normalize_oura_sleep(self, sleep: OuraSleepData) -> NormalizedSleepData:
        ...
        return NormalizedSleepData(
            ...
            user_id="",  # Will be set by the service
            raw_data={
                "day": str(sleep.day) if hasattr(sleep, "day") else None,
                "efficiency": (
                    sleep.efficiency if hasattr(sleep, "efficiency") else None
                ),
                "latency": sleep.latency if hasattr(sleep, "latency") else None,
                "hr_lowest": (
                    sleep.lowest_heart_rate
                    if hasattr(sleep, "lowest_heart_rate")
                    else None
                ),
                "hr_average": (
                    sleep.average_heart_rate
                    if hasattr(sleep, "average_heart_rate")
                    else None
                ),
                "hrv_average": (
                    sleep.average_hrv if hasattr(sleep, "average_hrv") else None
                ),
            },
        )

    def _normalize_oura_workout(
        self, activity: OuraActivityData
    ) -> NormalizedWorkoutData:
        """Normalize Oura workout data"""
        # Oura provides duration in seconds
        duration_minutes = activity.duration / 60 if activity.duration else None

        return NormalizedWorkoutData(
            workout_strain=None,  # Oura doesn't provide strain scores
            duration_minutes=duration_minutes,
            calories_burned=(
                float(activity.active_calories) if activity.active_calories else None
            ),
            average_heart_rate=(
                None  # Oura activity doesn't provide heart rate for daily activity
            ),
            max_heart_rate=(None),  # Oura activity doesn't provide max heart rate
            sport_type=activity.activity_type,
            start_time=activity.timestamp,
            end_time=activity.timestamp,
            device=WearableDevice.OURA_RING,
            device_specific_id=activity.activity_id,
            user_id="",  # Will be set by the service
            raw_data={
    ...
    def _oura_activity_to_metrics(
        ...
        metrics = []
        base_date = datetime.combine(activity.day, datetime.min.time())
        ...
        if activity.steps:
            metrics.append(
                NormalizedMetric(
                    ...
                    device_specific_id=activity.activity_id,
                    user_id="",
                    raw_data={"day": activity.day.isoformat(), "steps": activity.steps},
                )
            )
    ...
    def _normalize_garmin_recovery(
        self, daily_stats: GarminDailyStats
    ) -> NormalizedRecoveryData:
        """Normalize Garmin daily stats to recovery data"""
        # Garmin uses Body Battery as a recovery indicator
        # Higher body battery = better recovery
        recovery_score = None
        if daily_stats.body_battery_highest_value is not None:
            # Normalize body battery (0-100) to recovery score
            recovery_score = float(daily_stats.body_battery_highest_value)

        return NormalizedRecoveryData(
            recovery_score=recovery_score,
            hrv_score=None,  # Garmin doesn't provide HRV in daily stats
            resting_heart_rate=(
                float(daily_stats.resting_heart_rate)
                if daily_stats.resting_heart_rate
                else None
            ),
            sleep_need_hours=None,  # Garmin doesn't provide sleep need
            device=WearableDevice.GARMIN,
            device_specific_id=f"daily_{daily_stats.calendar_date}",
            timestamp=datetime.combine(daily_stats.calendar_date, datetime.min.time()),
            user_id="",  # Will be set by the service
            raw_data={
                "date": str(daily_stats.calendar_date),
                "body_battery_highest": daily_stats.body_battery_highest_value,
                "body_battery_lowest": daily_stats.body_battery_lowest_value,
                "stress_average": daily_stats.average_stress_level,
            },
        )

    def _normalize_garmin_sleep(self, sleep: GarminSleepData) -> NormalizedSleepData:
        """Normalize Garmin sleep data"""
        total_minutes = (
            sleep.sleep_duration_seconds / 60 if sleep.sleep_duration_seconds else None
        )

        return NormalizedSleepData(
            sleep_score=(
                float(sleep.sleep_quality_score) if sleep.sleep_quality_score else None
            ),
            total_duration_minutes=total_minutes,
            sleep_efficiency_percent=None,  # Garmin doesn't provide efficiency directly
            deep_sleep_minutes=(
            ...
            sleep_start=sleep.sleep_start_time,
            sleep_end=sleep.sleep_end_time,
            device=WearableDevice.GARMIN,
            device_specific_id=sleep.sleep_id,
            user_id="",
            raw_data={
                "date": str(sleep.calendar_date),
                "quality_score": sleep.sleep_quality_score,
                "average_spo2": sleep.average_sp_o2,
                "average_respiration": sleep.average_respiration,
                "average_stress": sleep.average_stress,
                "rest_heart_rate": sleep.rest_heart_rate,
            },
        )

    def _normalize_garmin_workout(
        self, activity: GarminActivityData
    ) -> NormalizedWorkoutData:
        """Normalize Garmin activity data"""
        duration_minutes = (
            activity.duration_seconds / 60 if activity.duration_seconds else 0
        )
        end_time = activity.start_time + timedelta(minutes=duration_minutes)

        return NormalizedWorkoutData(
            workout_strain=(
                float(activity.training_effect) if activity.training_effect else None
            ),
            duration_minutes=duration_minutes,
            calories_burned=(float(activity.calories) if activity.calories else None),
            average_heart_rate=(
            ...
            max_heart_rate=(
                float(activity.max_heart_rate) if activity.max_heart_rate else None
            ),
            sport_type=activity.activity_type,
    ...
    def _normalize_garmin_body_composition(
        self, body_comp: GarminBodyComposition
    ) -> List[NormalizedMetric]:
        """Convert Garmin body composition to metrics"""
        metrics = []
        base_time = datetime.combine(body_comp.measurement_date, datetime.min.time())

        # Weight metric
        if body_comp.weight_kg:
            metrics.append(
                NormalizedMetric(
                    device=WearableDevice.GARMIN,
                    metric_type=MetricType.WEIGHT,
                    value=body_comp.weight_kg,
                    unit="kg",
                    timestamp=base_time,
                    device_specific_id=f"weight_{body_comp.measurement_date}",
                    user_id="",
                    raw_data={"date": str(body_comp.measurement_date)},
                )
            )

        # Body fat percentage
        if body_comp.body_fat_percentage:
            metrics.append(
                NormalizedMetric(
                    device=WearableDevice.GARMIN,
                    metric_type=MetricType.BODY_FAT_PERCENTAGE,
                    value=body_comp.body_fat_percentage,
                    unit="%",
                    timestamp=base_time,
                    device_specific_id=f"bodyfat_{body_comp.measurement_date}",
                    user_id="",
                    raw_data={"date": str(body_comp.measurement_date)},
                )
            )
        ...
        # Muscle mass
        if body_comp.muscle_mass_kg:
            metrics.append(
                NormalizedMetric(
                    device=WearableDevice.GARMIN,
                    metric_type=MetricType.MUSCLE_MASS,
                    value=body_comp.muscle_mass_kg,
                    unit="kg",
                    timestamp=base_time,
                    device_specific_id=f"muscle_{body_comp.measurement_date}",
                    user_id="",
                    raw_data={"date": str(body_comp.measurement_date)},
                )
            )
