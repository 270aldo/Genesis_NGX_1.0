...
from .adapters.garmin import GarminAdapter, GarminConfig
...
class WearableIntegrationService:
    ...
    def _initialize_adapters(self):
        ...
        # Initialize Garmin adapter if configured
        if "garmin" in self.config:
            garmin_config = GarminConfig(
                consumer_key=self.config["garmin"]["consumer_key"],
                consumer_secret=self.config["garmin"]["consumer_secret"],
                redirect_uri=self.config["garmin"]["redirect_uri"],
            )
            self.device_adapters[WearableDevice.GARMIN] = garmin_config
            logger.info("Garmin adapter initialized")
    ...
    async def get_authorization_url(
        ...
        if device == WearableDevice.WHOOP:
            ...
            config = self.device_adapters[device]
            async with GarminAdapter(config) as garmin:
                auth_state = f"{user_id}:{state}" if state else user_id
                return garmin.get_auth_url(auth_state)
        else:
    ...
    async def complete_authorization(
        ...
        if device == WearableDevice.WHOOP:
            ...
            async with OuraAdapter(config) as oura:
                ...
                device_user_id = (
                    profile.user_id if profile.user_id else f"oura_{user_id}"
                )
    ...
    async def _sync_oura_data(
        ...
        # If not forcing refresh, only sync since last sync
        if not force_refresh and connection.last_sync:
            start_date = max(start_date, connection.last_sync.date())
        ...
        async with OuraAdapter(config) as oura:
            ...
            try:
                ...
                # Note: Oura doesn't have a separate workout API
                # Workouts are tracked as part of activity data
                # We already processed activity data above which includes workout info
