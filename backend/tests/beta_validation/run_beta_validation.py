...
from tests.beta_validation.intelligent_mock_client import (  # noqa: E402
    IntelligentMockOrchestratorClient,
)
from tests.beta_validation.real_orchestrator_client import (  # noqa: E402
    RealOrchestratorClient,
)
from tests.beta_validation.scenarios import (  # noqa: E402
...
from tests.beta_validation.test_config import TestEnvironment  # noqa: E402
from tests.beta_validation.validators import ResponseQualityValidator  # noqa: E402
...
class BetaValidationRunner:
    ...
    async def _create_orchestrator_client(self):
        ...
        if use_real_orchestrator:
            ...
            use_real_ai = self.config.get("use_real_ai", False)
            client = RealOrchestratorClient(test_mode=True, use_real_ai=use_real_ai)
    ...
    async def run_all_tests(
        ...
        Args:
            ...
            quick: Quick mode with reduced delays
        """
        self.config["quick_mode"] = quick
        ...
        if categories:
            test_categories = {
                k: v for k, v in test_categories.items() if k in categories
            }
        ...
        logger.info(
            f"Beta Validation Tests completed in {self.results['duration']:.2f} seconds"
        )
            ...
            try:
                ...
                if category_name == "ecosystem_integration":
                    scenario = scenario_class(
                        self.orchestrator_client, self.mcp_gateway_client
                    )
                else:
                    scenario = scenario_class(self.orchestrator_client)

                # Pass config to scenario if it supports it
                if hasattr(scenario, "set_config"):
                    scenario.set_config(self.config)
                ...
                # Update totals
                self.results["summary"]["total_scenarios"] += category_results.get(
                    "total_scenarios", 0
                )
                ...
                logger.info(
                    f"{category_name}: {category_results.get('passed', 0)}/{category_results.get('total_scenarios', 0)} passed"
                )
    ...
    async def _run_parallel_tests(self, test_categories: Dict[str, Any]):
        ...
        # Process results
        for (category_name, _), result in zip(
            test_categories.items(), category_results
        ):
            if isinstance(result, Exception):
                ...
                self.results["summary"]["total_scenarios"] += result.get(
                    "total_scenarios", 0
                )
    ...
    async def _run_category_async(self, category_name: str, scenario_class):
        ...
        logger.info(
            f"{category_name}: {result.get('passed', 0)}/{result.get('total_scenarios', 0)} passed"
        )
            ...
            for scenario in category_data["details"]:
                ...
                if not scenario.get("passed", False):
                    ...
                    for pattern in critical_patterns:
                        if (
                            pattern in scenario_name.lower()
                            or pattern in str(scenario.get("issues", [])).lower()
                        ):
        ...
        for category, data in self.results["categories"].items():
            if "pass_rate" in data:
                status = (
                    ...
                    else "⚠️" if data["pass_rate"] >= 70 else "❌"
                )
...
async def main():
    ...
    try:
        await runner.run_all_tests(
            categories=categories, parallel=args.parallel, quick=args.quick
        )
