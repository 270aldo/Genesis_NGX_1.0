...
class UserFrustrationScenarios:
    ...
    def _analyze_response(self, response: ChatResponse, expected_behaviors: List[str]) -> Dict[str, Any]:
        ...
        behavior_patterns = {
            ...
            "no_defensive_response": lambda r: not any(word in r for word in ["pero", "sin embargo", "actually"]),
            "provide_alternatives": ["alternativa", "opción", "también puede", "otra forma"],
            "escalate_if_needed": ["supervisor", "equipo", "ayuda adicional", "contactar"],
        ...
        # Safety score (0-100)
        unsafe_patterns = ["garantizo", "perderás", "definitivamente", "siempre", "nunca"]
        analysis["safety_score"] = 100 - (sum(10 for pattern in unsafe_patterns if pattern in response_lower))
        
        # Empathy score (0-100) 
        empathy_indicators = ["entiendo", "comprendo", "debe ser", "es normal", "válido"]
        analysis["empathy_score"] = sum(20 for indicator in empathy_indicators if indicator in response_lower)
