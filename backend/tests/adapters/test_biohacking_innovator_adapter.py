...
from infrastructure.adapters.biohacking_innovator_adapter import (
    ...
    with patch.object(
        ...
        mock_load_state.return_value = {
            "conversation_history": [
                {
                    "role": "user",
                    "content": "¿Qué suplementos me recomiendas para mejorar la cognición?",
                }
            ],
        ...
        context = await biohacking_innovator_adapter._get_context(
    ...
    with patch.object(
        ...
        test_context = {
            "conversation_history": [
                ...
                {
                    "role": "assistant",
                    "content": "Para mejorar la cognición, recomiendo...",
                },
            ],
            "user_profile": {"age": 35, "gender": "masculino"},
            "protocols": [
        ...
        # Llamar al método a probar
        await biohacking_innovator_adapter._update_context(
...
async def test_classify_query(biohacking_innovator_adapter):
    ...
    with patch.object(
        ...
        # Llamar al método a probar
        query_type = await biohacking_innovator_adapter._classify_query(
            "¿Qué suplementos puedo tomar para mejorar mi memoria?"
        )
...
async def test_run_async_impl_biohacking_protocol(biohacking_innovator_adapter):
    ...
    with (
        patch.object(
            biohacking_innovator_adapter, "_get_context", new_callable=AsyncMock
        ) as mock_get_context,
        patch.object(
            biohacking_innovator_adapter, "_update_context", new_callable=AsyncMock
        ) as mock_update_context,
        patch.object(
            biohacking_innovator_adapter, "_classify_query", new_callable=AsyncMock
