...
from infrastructure.adapters.intent_analyzer_adapter import (
    IntentAnalyzerAdapter,
...
def mock_original_analyzer():
    ...
    with patch(
        "infrastructure.adapters.intent_analyzer_adapter.intent_analyzer"
    ) as mock:
        ...
        mock.analyze_intent = AsyncMock(
            return_value=[
                Intent(
                    ...
                    entities=[
                        IntentEntity(
                            entity_type="exercise", value="push-up", confidence=0.95
                        )
                    ],
                )
            ]
        ...
        mock.analyze_intents_with_embeddings = AsyncMock(
            return_value=[
                Intent(
        ...
        mock.get_stats = AsyncMock(
            return_value={"total_queries": 10, "cached_embeddings": 5, "api_calls": 8}
        )
...
def mock_optimized_analyzer():
    ...
    with patch(
        "infrastructure.adapters.intent_analyzer_adapter.IntentAnalyzerOptimized"
    ) as mock_class:
        mock = MagicMock()
        mock_class.return_value = mock

        # Configurar el mock para devolver intenciones simuladas
        mock.analyze_query = AsyncMock(
            return_value=[
                Intent(
                    ...
                    entities=[
                        IntentEntity(
                            entity_type="metric", value="heart_rate", confidence=0.98
                        )
                    ],
                )
            ]
...
class TestIntentAnalyzerAdapter:
    ...
    async def test_error_handling_in_analyze_intent(self, adapter):
        ...
        adapter.use_optimized = False
        adapter._original_analyzer.analyze_intent.side_effect = Exception(
            "Error simulado"
        )

        # Analizar una consulta
        result = await adapter.analyze_content(
            user_query="Consulta que genera error", conversation_id="conv_error"
        )
    ...
    async def test_error_handling_in_analyze_intents_with_embeddings(self, adapter):
        ...
        # Configurar el analizador original para lanzar una excepción
        adapter.use_optimized = False
        adapter._original_analyzer.analyze_intents_with_embeddings.side_effect = (
