...
from agents.biohacking_innovator.agent import (
    BiohackingInnovator,
    BiohackingProtocolInput,
)
...
class TestBiohackingInnovator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with (
            patch(
                "agents.biohacking_innovator.agent.VertexAIClient",
                return_value=self.mock_gemini_client,
            ),
            patch(
                "agents.biohacking_innovator.agent.ProgramClassificationService",
    ...
    async def test_biohacking_protocol_skill_uses_program_context(self):
        ...
        biohacking_skill = next(
            (
                ...
                if skill.name == "biohacking_protocol"
            ),
            None,
        )
        ...
        prompt_calls = [
            call[0][0]
            for call in self.mock_gemini_client.generate_response.call_args_list
        ]
