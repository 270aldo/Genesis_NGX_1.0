...
from agents.motivation_behavior_coach.schemas import MotivationStrategiesInput
...
class TestMotivationBehaviorCoach(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with (
            patch(
                "agents.motivation_behavior_coach.agent.VertexAIClient",
                return_value=self.mock_gemini_client,
            ),
            patch(
                "agents.motivation_behavior_coach.agent.ProgramClassificationService",
    ...
    async def test_motivation_strategies_with_program_classification(self):
        ...
        self.assertTrue(
            "[PRIME]" in result.long_term_approach
            or "PRIME" in result.long_term_approach
        )
