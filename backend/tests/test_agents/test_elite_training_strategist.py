...
def mock_dependencies():
    ...
    with (
        patch(
            "agents.elite_training_strategist.agent.VertexAIClient",
            return_value=MockVertexAIClient(),
        ),
        patch(
            "agents.elite_training_strategist.agent.SupabaseClient",
            return_value=MockSupabaseClient(),
        ),
        patch(
            "agents.elite_training_strategist.agent.MCPToolkit",
            return_value=MockMCPToolkit(),
        ),
        patch(
            "agents.elite_training_strategist.agent.MCPClient",
...
async def test_elite_training_strategist_initialization(mock_dependencies):
    ...
    for skill_object, a2a_skill_def in zip(agent.skills, agent.a2a_skills):
        assert (
            a2a_skill_def["name"] == skill_object.name.replace("_", " ").title()
        )  # ADKAgent capitaliza y reemplaza guiones bajos
        ...
        if skill_object.input_schema:
            ...
            assert bool(
                input_mode["schema"]
            ), f"Input schema for {skill_object.name} is empty"
        ...
        if skill_object.output_schema:
            ...
            assert bool(
                output_mode["schema"]
            ), f"Output schema for {skill_object.name} is empty"
        else:
...
async def test_run_method_success(mock_dependencies):
    ...
    agent.vertex_ai_client = MockVertexAIClient()
...
async def test_run_method_with_error(mock_dependencies):
    ...
    with patch.object(
        MockVertexAIClient, "generate_response", side_effect=Exception("Error simulado")
    ):
        # Ejecutar el método run
...
async def test_prepare_context(mock_dependencies):
    ...
    user_input_example = (
        "Quiero un plan para mejorar mi resistencia en carrera de 10km."
    )
    ...
    context_input_example = {
        ...
        "very_long_key_to_be_ignored_hopefully": "x"
        * 300,  # Debería ser filtrado por longitud
    }

    # Escenario 1: Con perfil de usuario y contexto adicional
    prepared_context_full = agent._prepare_context(
        user_input_example, user_profile_example, context_input_example
    )
    ...
    assert (
        "Programa: PRIME" in prepared_context_full
    )  # Verificamos que _get_program_type_from_profile funciona
    ...
    assert (
        "preferred_training_days: [" in prepared_context_full
    )  # Chequeo flexible de la lista
    ...
    assert (
        f"Solicitud Específica del Usuario:\n{user_input_example}"
        in prepared_context_full
    )

    # Escenario 2: Sin perfil de usuario y sin contexto adicional
    prepared_context_minimal = agent._prepare_context(user_input_example, None, {})
