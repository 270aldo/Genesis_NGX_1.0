...
from tests.agents.test_base_agent import (
    BaseAgentTestSuite,
    NutritionAgentTestMixin,
    TrainingAgentTestMixin,
    WellnessAgentTestMixin,
)
...
class TestOrchestratorAgent(BaseAgentTestSuite):
    ...
    agent_class = NGXNexusOrchestrator
...
class TestAgentIntegration:
    ...
    def all_agents(self, mock_mcp_toolkit):
        ...
        return {
            "orchestrator": NGXNexusOrchestrator(mcp_toolkit=mock_mcp_toolkit),
            ...
            "stella": ProgressTracker(mcp_toolkit=mock_mcp_toolkit),
    ...
    @pytest.mark.asyncio
    async def test_orchestrator_can_route_to_all(
        self, all_agents, mock_vertex_ai_client
    ):
        ...
        for prompt, expected_agent in routing_tests:
            ...
            result = await orchestrator.route_request(
                prompt=prompt, user_context={"user_id": "test_123"}
            )
...
class TestAgentPerformance:
    ...
    @pytest.mark.asyncio
    async def test_response_time_all_agents(
        self, mock_mcp_toolkit, mock_vertex_ai_client
    ):
        ...
        for AgentClass in agents_to_test:
            ...
            start = time.time()
            await agent.process(
                prompt="Test rápido", user_context={"user_id": "test_123"}
            )
