...
def mocked_agent():
    ...
    agent.vertex_ai_client = mock_gemini_client
...
async def test_skill_generate_training_plan(mocked_agent):
    ...
    # y _generate_training_plan_logic llama a vertex_ai_client.generate_content.
    # El mock de _get_context se usa indirectamente por _generate_training_plan_logic.

    # Si vertex_ai_client.generate_content fue llamado, significa que la lógica interna de la skill se ejecutó.
    agent.vertex_ai_client.generate_structured_output.assert_called_once()
    call_args, call_kwargs = agent.vertex_ai_client.generate_structured_output.call_args
    prompt = call_args[0]
