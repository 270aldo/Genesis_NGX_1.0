...
class TestOrchestratorCore:
    ...
    def orchestrator(self, mock_vertex_ai_client, mock_mcp_toolkit, mock_logger):
        ...
        with patch('agents.orchestrator.agent.get_logger', return_value=mock_logger):
            with patch('agents.orchestrator.agent.VertexAIClient', return_value=mock_vertex_ai_client):
                agent = OrchestratorAgent(mcp_toolkit=mock_mcp_toolkit)
                return agent
    ...
    @pytest.mark.asyncio
    async def test_route_request_to_nutrition(self, orchestrator, mock_vertex_ai_client):
        ...
        result = await orchestrator.route_request(
            prompt="¿Qué debo comer antes de entrenar?",
    ...
    async def test_route_request_to_training(self, orchestrator, mock_vertex_ai_client):
        ...
        result = await orchestrator.route_request(
            prompt="¿Cómo mejorar mi sentadilla?",
            user_context={"user_id": "123"}
        )
    ...
    @pytest.mark.asyncio
    async def test_coordinate_multiple_agents(self, orchestrator, mock_vertex_ai_client):
        """Test de coordinación de múltiples agentes"""
        # Mock de agentes disponibles
        mock_sage = MagicMock()
        mock_sage.process = AsyncMock(return_value={
            "response": "Consume 30g de proteína",
            "confidence": 0.9
        })
        
        mock_nexus = MagicMock()
        mock_nexus.process = AsyncMock(return_value={
    ...
    @pytest.mark.asyncio
    async def test_error_handling_invalid_json(self, orchestrator, mock_vertex_ai_client):
    ...
    @pytest.mark.asyncio
    async def test_agent_selection_with_context(self, orchestrator, mock_vertex_ai_client):
        ...
        result = await orchestrator.route_request(
            prompt="Necesito ayuda con mi ciclo",
...
class TestOrchestratorValidation:
    ...
    def orchestrator(self, mock_vertex_ai_client, mock_mcp_toolkit):
        ...
        return OrchestratorAgent(mcp_toolkit=mock_mcp_toolkit)
    ...
    async def test_validate_agent_availability(self, orchestrator):
        ...
        with patch('agents.orchestrator.agent.AgentRegistry') as mock_registry:
            ...
            result = await orchestrator.validate_agent_availability("non_existent_agent")
