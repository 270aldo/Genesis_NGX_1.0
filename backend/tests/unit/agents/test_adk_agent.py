...
class TestADKAgent:
    ...
    async def test_skill_chaining(self, adk_agent):
        ...
        result2 = await adk_agent.execute_skill("skill2", {"step": 2, "previous": result1})
    ...
    def test_skill_discovery(self, adk_agent):
        ...
        discovered_skills = [
            MockSkill("auto_skill1"),
            MockSkill("auto_skill2")
        ]
...
class TestADKAgentAsync:
    ...
    async def test_concurrent_skill_execution(self, adk_agent):
        ...
        tasks = [
            adk_agent.execute_skill("concurrent_skill", {"task": i})
            for i in range(5)
        ]
    ...
    async def test_skill_timeout(self, adk_agent):
        ...
        with pytest.raises(asyncio.TimeoutError):
            await asyncio.wait_for(
                adk_agent.execute_skill("slow_skill", {}),
                timeout=0.1
            )