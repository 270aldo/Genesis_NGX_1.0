...
@app.task(base=CriticalTask, name="send_adherence_alert")
def send_adherence_alert(outreach_data: Dict[str, Any]) -> Dict[str, Any]:
    """
    Send adherence alert and trigger agent outreach.

    Args:
        outreach_data: Data for agent outreach including user_id, agent_id, context
    ...
    try:
        logger.info(
            f"Processing adherence alert for user {outreach_data.get('user_id')}"
        )

        # Store alert in database for agent processing
        supabase = SupabaseClient()

        alert_record = {
            "user_id": outreach_data["user_id"],
            "agent_id": outreach_data["agent_id"],
            "alert_type": "adherence_risk",
            "priority": outreach_data.get("priority", "medium"),
            "context": outreach_data.get("context", {}),
            "suggested_approach": outreach_data.get("suggested_approach", ""),
            "timeline": outreach_data.get("timeline", "within_48_hours"),
            "status": "pending",
            "created_at": datetime.utcnow().isoformat(),
            "due_at": _calculate_due_date(
                outreach_data.get("timeline", "within_48_hours")
            ),
        }

        # Insert alert record
        result = supabase.table("adherence_alerts").insert(alert_record).execute()

        if result.data:
            alert_id = result.data[0]["id"]
            logger.info(
                f"Adherence alert {alert_id} created for user {outreach_data['user_id']}"
            )

            # Send notification to agent system
            _notify_agent_system(alert_record)
        ...
        logger.error(f"Adherence alert failed: {e}")
        raise


def _calculate_due_date(timeline: str) -> str:
    """Calculate due date based on timeline string."""
    from datetime import timedelta

    timeline_mapping = {
        "immediate": timedelta(hours=1),
        "within_24_hours": timedelta(hours=24),
        "within_48_hours": timedelta(hours=48),
        "within_week": timedelta(days=7),
    }

    delta = timeline_mapping.get(timeline, timedelta(hours=48))
    due_date = datetime.utcnow() + delta

    return due_date.isoformat()


def _notify_agent_system(alert_record: Dict[str, Any]) -> None:
    """Notify agent system of new adherence alert."""
    try:
        # This would integrate with the agent system to notify the specific agent
        # For now, log the notification
        logger.info(
            f"AGENT NOTIFICATION: {alert_record['agent_id']} has new adherence alert "
            f"for user {alert_record['user_id']} with priority {alert_record['priority']}"
        )

        # Could trigger push notification to agent dashboard
        # Could send email/SMS to agent
        # Could create in-app notification

    except Exception as e:
        logger.warning(f"Failed to notify agent system: {e}")


@app.task(base=CriticalTask, name="process_adherence_batch")
def process_adherence_batch(user_batch: List[str]) -> Dict[str, Any]:
    """
    Process adherence monitoring for a batch of users.

    Args:
        user_batch: List of user IDs to process
    ...
    try:
        ...
        from core.adherence_prediction_engine import AdherenceMetrics

        monitoring_service = AdherenceMonitoringService()
        results = {
            "processed_users": [],
            "failed_users": [],
        ...
        for user_id in user_batch:
            try:
                # Get user metrics (would fetch from database)
                ...
                results["processed_users"].append(
                    {
                        ...
                        "interventions_triggered": len(
                            monitoring_result["interventions_triggered"]
                        ),
                    }
                )

                results["total_interventions"] += len(
                    monitoring_result["interventions_triggered"]
                )

                # Track high risk users
                if monitoring_result["prediction"].risk_level.value in [
                    "high",
                    "very_high",
                ]:
                    results["high_risk_users"].append(user_id)
        ...
        logger.info(
            f"Adherence batch processing completed: "
            f"{len(results['processed_users'])} processed, "
            f"{len(results['failed_users'])} failed, "
            f"{results['total_interventions']} interventions triggered"
        )

        return results

    except Exception as e:
        logger.error(f"Adherence batch processing failed: {e}")
        raise


def _get_user_metrics(user_id: str) -> AdherenceMetrics:
    """Get user metrics for adherence calculation."""
    # This would fetch real user data from database
    # For now, return sample metrics
    from core.adherence_prediction_engine import AdherenceMetrics

    return AdherenceMetrics(
        daily_app_usage_minutes=25.0,
        weekly_active_days=5,
        agent_interaction_frequency=3.2,
        message_response_time_hours=4.5,
        consistency_score=0.75,
        goal_completion_rate=0.82,
        self_reporting_frequency=4.1,
        protocol_modification_requests=2,
        progress_satisfaction_score=7.5,
        milestone_achievement_rate=0.78,
        plateau_duration_days=12,
        expectation_reality_gap=0.1,
        support_system_strength=0.8,
        environmental_challenges=3,
        competing_priorities=4,
        previous_program_completion_rate=0.85,
        longest_adherence_streak_days=75,
        average_dropout_timeframe_days=120,
    )


def _get_user_context(user_id: str) -> Dict[str, Any]:
    """Get user context for adherence calculation."""
    # This would fetch real user context from database
    ...
    return {
        ...
        "social_support_changes": False,
        "goal_alignment": "high",
        "time_constraints_increased": False,
    }
