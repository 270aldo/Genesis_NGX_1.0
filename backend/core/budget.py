...
class TokenUsage(BaseModel):
    ...
    completion_tokens: int = Field(
        default=0, description="Tokens utilizados en completions"
    )
...
class AgentBudget(BaseModel):
    ...
    max_tokens: int = Field(
        ..., description="Máximo de tokens permitidos en el período"
    )
    period: BudgetPeriod = Field(
        default=BudgetPeriod.MONTHLY, description="Período del presupuesto"
    )
    action_on_limit: BudgetAction = Field(
        default=BudgetAction.WARN, description="Acción al alcanzar el límite"
    )
    fallback_model: Optional[str] = Field(
        default=None, description="Modelo alternativo para modo degradado"
    )
    reset_day: Optional[int] = Field(
        default=1, description="Día del mes/semana para reset (1-31 o 1-7)"
    )
...
class BudgetManager:
    ...
    def __init__(self, persistence_client=None):
        ...
        self.usage: Dict[str, Dict[str, TokenUsage]] = (
            {}
        )  # agent_id -> {period_key -> usage}
    ...
    def _load_budgets(self) -> None:
        ...
        if not os.path.exists(config_path):
            logger.warning(
                f"Archivo de configuración de presupuestos no encontrado: {config_path}"
            )
            return
        ...
        try:
            ...
            for agent_id, agent_config in agents_config.items():
                ...
                budget = AgentBudget(
                    ...
                    action_on_limit=merged_config.get(
                        "action_on_limit", settings.default_budget_action
                    ),
    ...
    def set_budget(self, budget: AgentBudget) -> None:
        ...
        logger.info(
            f"Presupuesto establecido para agente {budget.agent_id}: {budget.max_tokens} tokens/{budget.period}"
        )
    ...
    def _should_reset(self, agent_id: str) -> bool:
        ...
        if budget.period == BudgetPeriod.DAILY:
            ...
            reset_day = min(
                budget.reset_day or 1, 28
            )  # Usar 28 como máximo para compatibilidad con febrero
            return now.day == reset_day and (
        ...
        if agent_id in self.usage:
            ...
            if self.persistence_client:
                try:
                    ...
                    logger.error(
                        f"Error al archivar uso de tokens para {agent_id}: {e}"
                    )
    ...
    async def record_usage(
        self, agent_id: str, prompt_tokens: int, completion_tokens: int, model: str
        ...
        async with self._lock:
            ...
            if usage.total_tokens > budget.max_tokens:
                ...
                if budget.action_on_limit == BudgetAction.BLOCK:
                    ...
                    budget.action_on_limit == BudgetAction.DEGRADE
                    and budget.fallback_model
                ):
                    ...
                    # Encolar la tarea para procesamiento posterior cuando haya presupuesto
                    from tasks.budget import queue_over_budget_task
                    ...
                    # Enviar a cola de baja prioridad para procesamiento cuando haya presupuesto
                    try:
                        queue_over_budget_task.apply_async(
                            args=[task_data], queue="low_priority", priority=1
                        )
                        logger.info(
                            f"Tarea encolada para agente {agent_id} debido a límite de presupuesto"
                        )
                    except Exception as e:
                        logger.error(f"Error al encolar tarea para {agent_id}: {e}")

                    return False, None  # Bloquear la ejecución inmediata
