...
class Settings(BaseSettings):
    ...
    ORCHESTRATOR_DEFAULT_MODEL_ID: str = Field(
        default="gemini-2.5-pro", 
        json_schema_extra={"env": "ORCHESTRATOR_DEFAULT_MODEL_ID"}
    )

    # Configuración de A2A
    A2A_HOST: str = Field(default="localhost", json_schema_extra={"env": "A2A_HOST"})
    A2A_PORT: int = Field(default=9000, json_schema_extra={"env": "A2A_PORT"})
    a2a_server_url: AnyUrl = Field(
        default="http://localhost:9000", json_schema_extra={"env": "A2A_SERVER_URL"}
    )
    a2a_websocket_url: str = Field(
        default="ws://localhost:9000/ws", json_schema_extra={"env": "A2A_WEBSOCKET_URL"}
    )
    ...
    gcp_region: str = Field(
        default="us-central1", json_schema_extra={"env": "GCP_REGION"}
    )
    vertex_project_id: Optional[str] = Field(
        default=None, json_schema_extra={"env": "VERTEX_PROJECT_ID"}
    )
    vertex_location: str = Field(
        default="us-central1", json_schema_extra={"env": "VERTEX_LOCATION"}
    )
    ...
    # Configuración de Celery para notificaciones programadas
    celery_broker_url: str = Field(
        default="redis://localhost:6379/0",
        json_schema_extra={"env": "CELERY_BROKER_URL"},
    )
    celery_result_backend: str = Field(
        default="redis://localhost:6379/0",
        json_schema_extra={"env": "CELERY_RESULT_BACKEND"},
    )
