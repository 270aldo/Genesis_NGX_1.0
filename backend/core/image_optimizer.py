...
class ImageOptimizer:
    ...
    def __init__(
        ...
        Args:
            ...
            telemetry: Instancia de Any para métricas y trazas (opcional)
        """
        ...
        logger.info(
            f"ImageOptimizer inicializado con max_width={max_width}, max_height={max_height}, quality={quality}"
        )
    ...
    async def optimize_image(
        ...
        if self.telemetry:
            ...
            self.telemetry.add_span_attribute(
                span, "preserve_text_quality", preserve_text_quality
            )
            if force_format:
        ...
        try:
            ...
            if optimized_size >= original_size:
                logger.debug(
                    "La imagen optimizada es más grande que la original, usando la original"
                )
                ...
                if self.telemetry:
                    self.telemetry.add_span_attribute(
                        span, "optimization_reverted", True
                    )
            ...
            if self.telemetry:
                self.telemetry.add_span_attribute(
                    span, "optimized_size", optimized_size
                )
                self.telemetry.add_span_attribute(span, "bytes_saved", bytes_saved)
                self.telemetry.add_span_attribute(
                    span, "compression_ratio", metadata["compression_ratio"]
                )
                self.telemetry.record_metric("image_optimizer.bytes_saved", bytes_saved)
                self.telemetry.record_metric(
                    "image_optimizer.compression_ratio", metadata["compression_ratio"]
                )

            logger.debug(
                f"Imagen optimizada: {original_size} -> {optimized_size} bytes ({bytes_saved} bytes ahorrados)"
            )
                ...
                try:
                    ...
                    if os.path.exists(image_data):
                        with open(image_data, "rb") as f:
                            data = f.read()
                        format_detected = (
                            os.path.splitext(image_data)[1].lower().replace(".", "")
                        )
                        ...
                        raise ValueError(
                            f"No se pudo procesar la entrada de imagen: {image_data[:30]}..."
                        )
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(image_data["url"]) as response:
                        if response.status == 200:
                            ...
                            url_path = image_data["url"].split("?")[
                                0
                            ]  # Eliminar parámetros de consulta
                            format_detected = (
                                os.path.splitext(url_path)[1].lower().replace(".", "")
                            )
                            if not format_detected:
                                format_detected = "jpeg"  # Asumir JPEG
                            return data, format_detected
    ...
    async def _detect_text_heuristic(self, img: Image.Image) -> bool:
        ...
        if gray_img.width > 1000 or gray_img.height > 1000:
            ...
            analysis_img = gray_img.resize(
                (int(gray_img.width * ratio), int(gray_img.height * ratio)),
                Image.LANCZOS,
            )
        else:
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            ...
            "average_saving_per_image": (
                self.stats["bytes_saved"] / self.stats["images_processed"]
