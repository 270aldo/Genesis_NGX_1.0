...
try:
    from agents.elite_training_strategist.agent import EliteTrainingStrategistAgent
    from agents.precision_nutrition_architect.agent import PrecisionNutritionArchitectAgent
    from agents.female_wellness_coach.agent import FemaleWellnessCoachAgent
    from agents.progress_tracker.agent import ProgressTrackerAgent
    from agents.motivation_behavior_coach.agent import MotivationBehaviorCoachAgent
    from agents.nova_biohacking_innovator.agent import NovaBiohackingInnovatorAgent
    from agents.wave_performance_analytics.agent import WAVEPerformanceAnalytics
    from agents.code_genetic_specialist.agent import CodeGeneticSpecialistAgent
...
AGENTS_TO_TEST = [
    ("BLAZE", EliteTrainingStrategistAgent),
    ("SAGE", PrecisionNutritionArchitectAgent),
    ("LUNA", FemaleWellnessCoachAgent),
    ("STELLA", ProgressTrackerAgent),
    ("SPARK", MotivationBehaviorCoachAgent),
    ("NOVA", NovaBiohackingInnovatorAgent),
    ("WAVE", WAVEPerformanceAnalytics),
    ("CODE", CodeGeneticSpecialistAgent),
]
    ...
    for agent_name, agent_class in AGENTS_TO_TEST:
        try:
            ...
            if hasattr(agent_class, 'DEFAULT_INSTRUCTION'):
                print(f"  ✓ DEFAULT_INSTRUCTION encontrado")
            else:
                print(f"  ⚠️  DEFAULT_INSTRUCTION no encontrado directamente")
            ...
            try:
                ...
                if agent_name == "BLAZE":
                    from agents.elite_training_strategist.enhanced_prompt import get_enhanced_blaze_prompt
                    ...
                    from agents.female_wellness_coach.enhanced_prompt import get_enhanced_luna_prompt
                    prompt = get_enhanced_luna_prompt()
                elif agent_name == "STELLA":
                    from agents.progress_tracker.enhanced_prompt import get_enhanced_stella_prompt
                    prompt = get_enhanced_stella_prompt()
                elif agent_name == "SPARK":
                    from agents.motivation_behavior_coach.enhanced_prompt import get_enhanced_spark_prompt
                    prompt = get_enhanced_spark_prompt()
                elif agent_name == "NOVA":
                    from agents.nova_biohacking_innovator.enhanced_prompt import get_enhanced_nova_prompt
                    prompt = get_enhanced_nova_prompt()
                elif agent_name == "WAVE":
                    from agents.wave_performance_analytics.enhanced_prompt import get_enhanced_wave_prompt
                    prompt = get_enhanced_wave_prompt()
                elif agent_name == "CODE":
                    from agents.code_genetic_specialist.enhanced_prompt import get_enhanced_code_prompt
                ...
                if "CONSIDERACIONES DE SEGURIDAD" in prompt:
                    print(f"  ✓ Sección de seguridad presente en el prompt")
                else:
                    print(f"  ⚠️  Sección de seguridad no encontrada explícitamente")
                ...
                if "EJEMPLOS DE INTERACCIONES SEGURAS" in prompt:
                    print(f"  ✓ Ejemplos few-shot presentes")
                else:
                    print(f"  ⚠️  Ejemplos few-shot no encontrados")
...
def test_safety_validation():
    ...
    test_cases = [
        ...
        ("Es importante mantener una dieta balanceada y consultar con un nutricionista.", True),
    ]
    ...
    for response, should_be_safe in test_cases:
        ...
        if is_safe == should_be_safe:
            ...
            print(f"✗ Caso incorrecto: Se esperaba {'Seguro' if should_be_safe else 'No seguro'}, pero fue {'Seguro' if is_safe else 'No seguro'}")
            if issues:
