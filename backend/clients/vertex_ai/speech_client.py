...
class VertexAISpeechClient:
    ...
    def __init__(self, model: str = "chirp", telemetry: Optional[Any] = None):
        """
        ...
        try:
            ...
            logger.info(
                f"Inicializando Vertex AI para VertexAISpeechClient con Proyecto: {gcp_project_id}, Región: {gcp_region}"
            )
            aiplatform.init(project=gcp_project_id, location=gcp_region)
            self.vertex_ai_initialized = True
            logger.info(
                "Vertex AI inicializado correctamente para VertexAISpeechClient"
            )
        except Exception as e:
            logger.error(
                f"Error al inicializar Vertex AI para VertexAISpeechClient: {e}",
                exc_info=True,
            )

    async def transcribe_audio(
        self, audio_data: Union[str, Dict[str, Any]], language_code: str = "es-ES"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if self.vertex_ai_initialized:
                ...
                config = {"language_codes": [language_code], "model": self.model}

                # Crear la solicitud
                request = {"audio": {"content": processed_audio}, "config": config}
                ...
                logger.warning(
                    "Vertex AI no inicializado. Usando transcripción simulada."
                )
                result = {
            ...
            if self.telemetry and span:
                ...
                self.telemetry.record_metric(
                    "vertex_ai_speech_transcribe_duration", duration
                )
            ...
            if self.telemetry and span:
                ...
                self.telemetry.record_metric(
                    "vertex_ai_speech_transcribe_error_count", 1
                )
    ...
    async def synthesize_speech(
        ...
        try:
            ...
            if self.telemetry and span:
                ...
                # Registrar métricas
                self.telemetry.record_metric(
                    "vertex_ai_speech_synthesize_duration", duration
                )
            ...
            if self.telemetry and span:
                ...
                # Registrar métricas de error
                self.telemetry.record_metric(
                    "vertex_ai_speech_synthesize_error_count", 1
                )
    ...
    async def analyze_audio(
        self,
        audio_data: Union[str, Dict[str, Any]],
        ...
        try:
            ...
            if self.vertex_ai_initialized:
                ...
                # Simulación para desarrollo/pruebas
                logger.warning(
                    "Vertex AI no inicializado. Usando análisis de audio simulado."
                )
                result = {
            ...
            if self.telemetry and span:
                ...
                # Registrar métricas
                self.telemetry.record_metric(
                    "vertex_ai_speech_analyze_duration", duration
                )
        ...
        try:
            ...
            if analysis_type == "emotion":
                ...
                emotion_match = re.search(
                    r"predominante.*?es\s+([a-zA-ZáéíóúÁÉÍÓÚñÑ]+)",
                    analysis_text,
                    re.IGNORECASE,
                )
                predominant = (
                    emotion_match.group(1).lower() if emotion_match else "neutral"
                )

                # Buscar puntuaciones
                for emotion in emotions.keys():
                    score_match = re.search(
                        f"{emotion}[:\s]+([0-9.]+)", analysis_text, re.IGNORECASE
                    )
                    if score_match:
                        try:
                ...
                return {"predominant": predominant, "scores": emotions}
                ...
                intent_match = re.search(
                    r"intención.*?es\s+([a-zA-ZáéíóúÁÉÍÓÚñÑ]+)",
                    analysis_text,
                    re.IGNORECASE,
                )
                ...
                for intent in intents.keys():
                    score_match = re.search(
                        f"{intent}[:\s]+([0-9.]+)", analysis_text, re.IGNORECASE
                    )
                    if score_match:
                        try:
                ...
                return {"predominant": predominant, "scores": intents}
                ...
                segment_matches = re.finditer(
                    r"Hablante\s+(\d+):\s+\[([0-9:]+)\s*-\s*([0-9:]+)\]\s+(.*?)(?=Hablante\s+\d+:|$)",
                    analysis_text,
                    re.DOTALL,
                )
            ...
            logger.error(
                f"Error al estructurar respuesta de análisis: {e}", exc_info=True
            )
            return {"text": analysis_text, "error": str(e)}
