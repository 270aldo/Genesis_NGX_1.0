...
from core.logging_config import (
    get_logger,
)  # Asegurándonos que el logger sea el correcto
...
class CacheManager:
    ...
    def __init__(
        ...
        ]  # Lista de diccionarios por partición
        ...
        if self.use_redis:
            # Redis se inicializará de forma lazy desde el pool cuando sea necesario
            self.redis_client = None
            self._redis_url = redis_url or os.environ.get(
                "REDIS_URL", "redis://localhost:6379/0"
            )
            logger.info(
                "Redis configurado para usar connection pool. La conexión se establecerá en la primera operación."
            )

    async def _ensure_redis_client(self):
        """Asegura que el cliente Redis esté inicializado desde el pool."""
            ...
            try:
                if redis_pool_manager:
                    ...
                    if self.redis_client:
                        ...
                        logger.info("Cliente Redis obtenido del pool exitosamente.")
                        return True
                    else:
                        # Fallback: crear cliente directo si el pool no está disponible
                        self.redis_client = redis.Redis.from_url(self._redis_url)
                        await self.redis_client.ping()
                        logger.info(
                            "Cliente Redis creado directamente (pool no disponible)."
                        )
                        return True
                else:
                    self.use_redis = False
                    self.l2_enabled = False
                    logger.warning("Redis pool manager no disponible.")
                    return False
            except Exception as e:
                logger.error(
                    f"No se pudo conectar a Redis: {e}. Desactivando uso de Redis para esta instancia."
                )
                self.use_redis = False
                self.l2_enabled = False
    ...
    async def get(self, key: str, default=None) -> Any:
        ...
        try:
            with (
                telemetry.start_span("cache.get")
            ...
            async with lock:
                ...
                if key in self.memory_cache[partition]:
                    ...
                    if time.time() - entry["timestamp"] > self.ttl:
                        ...
                        self.stats["partitions"][partition]["items"] -= 1
                        self.stats["partitions"][partition]["bytes"] -= entry[
                            "size_bytes"
                        ]
                        self.stats["current_items"]["l1"] -= 1
    ...
    async def set(
        ...
        try:
            with (
                telemetry.start_span("cache.set")
                if self.enable_telemetry
                else nullcontext()
            ) as span:
                if self.enable_telemetry and span:
                ...
                value_data, original_size_bytes, final_size_bytes, is_compressed = (
                    await self._prepare_value_for_storage(value)
                )
                    ...
                    if await self._ensure_redis_connected():
                        try:
                            ...
                            await self.redis_client.set(
                                key, redis_value, ex=ttl or self.ttl
                            )
                            ...
                            if isinstance(e, redis.exceptions.ConnectionError):
                                logger.warning(
                                    "Desactivando Redis debido a error de conexión."
                                )
                                self.l2_enabled = False
                ...
                if prefetch_related and pattern and "*" in pattern:
                    ...
                    try:
                        ...
                        if (
                            ...
                            and await self._ensure_redis_connected()
                        ):
                            related_keys = await self.redis_client.keys(
                                f"{pattern_base}*"
                            )
                            if (
                                related_keys and len(related_keys) <= 10
                            ):  # Limitar a 10 claves para evitar sobrecarga
                                # Excluir la clave actual
                                related_keys = [k for k in related_keys if k != key]
                                if related_keys:
                                    asyncio.create_task(
                                        self._prefetch_related_keys(related_keys)
                                    )
                                    self.stats["prefetch_requests"] += 1
                    except Exception as e:
                        logger.debug(
                            f"Error en prefetching: {e}"
                        )  # Debug porque no es crítico
    ...
    async def _prepare_value_for_storage(
        self, value: Any
        ...
        if original_size_bytes > self.compression_threshold:
            try:
                ...
                compressed_data = zlib.compress(
                    serialized_value.encode("utf-8"), self.compression_level
                )
                ...
                if compressed_size < original_size_bytes:
                    ...
                    self.stats["compression"]["savings_bytes"] += (
                        original_size_bytes - final_size_bytes
                    )
                    self.stats["compression"]["compressed_items"] += 1
                    if self.stats["compression"]["compressed_items"] > 0:
                        self.stats["compression"]["compression_ratio"] = self.stats[
                            "compression"
                        ]["savings_bytes"] / sum(
                            [
                                self.stats["current_memory_bytes"]["l1"],
                                self.stats["compression"]["savings_bytes"],
                            ]
                        )
    ...
    async def invalidate_pattern(self, pattern: str) -> int:
        ...
        """
        with (
            telemetry.start_span("cache.invalidate_pattern")
            if self.enable_telemetry
            else nullcontext()
        ) as span:
            if self.enable_telemetry and span:
            ...
            if pattern in self.pattern_subscriptions:
                keys_to_invalidate = self.pattern_subscriptions[pattern].get(
                    "keys", set()
                )
                if keys_to_invalidate:
                    for key in list(
                        keys_to_invalidate
                    ):  # Usar lista para evitar modificar durante iteración
                    ...
                    async with self.locks[partition_idx]:
                        ...
                        for key in keys_to_delete:
                            if key in self.memory_cache[partition_idx]:
                                size_bytes = self.memory_cache[partition_idx][key].get(
                                    "size_bytes", 0
                                )
                                del self.memory_cache[partition_idx][key]
                                self.memory_cache_current_bytes -= size_bytes
                                self.stats["partitions"][partition_idx][
                                    "bytes"
                                ] -= size_bytes
    ...
    async def _cleanup_if_needed(self, needed_space_bytes: int = 0):
        ...
        """
        with (
            telemetry.start_span("cache.cleanup")
            if self.enable_telemetry
            else nullcontext()
        ) as span:
            if self.enable_telemetry and span:
            ...
            self.stats["current_items"][
                "total"
            ] = total_items  # Aproximación, no incluye L2
        ...
        if expired_count > 0:
            ...
            if self.enable_telemetry:
                telemetry.record_event(
                    "cache",
                    "expired_eviction",
                    {"count": expired_count, "bytes": expired_bytes},
                )
    ...
    async def _apply_lru_policy(self, needed_space_bytes: int):
        ...
        while (
            self.memory_cache_current_bytes + needed_space_bytes > self.l1_max_bytes
        ...
        if evicted_count > 0:
            ...
            if self.enable_telemetry:
                telemetry.record_event(
                    "cache",
                    "lru_eviction",
                    {"count": evicted_count, "bytes": evicted_bytes},
                )
    ...
    async def _apply_lfu_policy(self, needed_space_bytes: int):
        ...
        for lfu_key, _ in sorted_keys:
            if (
                self.memory_cache_current_bytes + needed_space_bytes
                <= self.l1_max_bytes
            ):
                break
    ...
    async def flush(self):
        ...
        try:
            with (
                telemetry.start_span("cache.flush")
                if self.enable_telemetry
                else nullcontext()
            ):
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            with (
                telemetry.start_span("cache.get_stats")
                if self.enable_telemetry
                else nullcontext()
            ):
                ...
                self.stats["current_memory_bytes"][
                    "l1"
                ] = self.memory_cache_current_bytes
                ...
                for p in range(self.partitions):
                    cache_stats["partitions"][str(p)] = self.stats["partitions"][
                        p
                    ].copy()

                # Calcular ratios de aciertos
                total_requests_l1 = (
                    cache_stats["hits"]["l1"] + cache_stats["misses"]["l1"]
                )
                total_requests_l2 = (
                    cache_stats["hits"]["l2"] + cache_stats["misses"]["l2"]
                )
                total_requests = (
                    cache_stats["hits"]["total"] + cache_stats["misses"]["total"]
                )

                cache_stats["hit_ratio"] = {
                    "l1": (
                        cache_stats["hits"]["l1"] / total_requests_l1
                        if total_requests_l1 > 0
                        else 0
                    ),
                    "l2": (
                        cache_stats["hits"]["l2"] / total_requests_l2
                        if total_requests_l2 > 0
                        else 0
                    ),
                    "total": (
                        cache_stats["hits"]["total"] / total_requests
                        if total_requests > 0
                        else 0
                    ),
                }
                ...
                if (
                    ...
                    try:
                        ...
                        cache_stats["current_items"]["l2"] = self.stats[
                            "current_items"
                        ]["l2"]
                        cache_stats["current_items"]["total"] = (
                            cache_stats["current_items"]["l1"]
                            + cache_stats["current_items"]["l2"]
                        )

                    except Exception as e:
                        logger.warning(
                            f"No se pudieron obtener estadísticas de Redis: {e}"
                        )
    ...
    async def _ensure_redis_connected(self) -> bool:
        """Asegura que el cliente Redis esté conectado y funcionando."""
        # Primero asegurar que tenemos un cliente
        if not await self._ensure_redis_client():
            return False

        # Luego verificar que esté funcionando
        try:
            await self.redis_client.ping()
            return True
        except redis.exceptions.ConnectionError as e:
            logger.error(
                f"Redis connection error: {e}. Disabling Redis for this session."
            )
            ...
            logger.error(
                f"Unexpected Redis error: {e}. Disabling Redis for this session."
            )
            self.use_redis = False
            self.stats["errors"] += 1
            return False
