...
class VertexAIMultimodalClient:
    ...
    def __init__(
        ...
        try:
            ...
            logger.info(
                f"Inicializando Vertex AI para VertexAIMultimodalClient con Proyecto: {gcp_project_id}, Región: {gcp_region}"
            )
            aiplatform.init(project=gcp_project_id, location=gcp_region)
            self.vertex_ai_initialized = True
            logger.info(
                "Vertex AI inicializado correctamente para VertexAIMultimodalClient"
            )
        except Exception as e:
            logger.error(
                f"Error al inicializar Vertex AI para VertexAIMultimodalClient: {e}",
                exc_info=True,
            )
    ...
    async def process_multimodal(
        ...
        if self.telemetry:
            ...
            self.telemetry.add_span_attribute(
                span, "max_output_tokens", max_output_tokens
            )
        ...
        try:
            ...
            if self.telemetry and span:
                ...
                self.telemetry.record_metric(
                    "vertex_ai_multimodal_process_duration", duration
                )
            ...
            if self.telemetry and span:
                ...
                self.telemetry.record_metric(
                    "vertex_ai_multimodal_process_error_count", 1
                )
    ...
    async def compare_images(
        ...
        try:
            ...
            if self.telemetry and span:
                ...
                # Registrar métricas
                self.telemetry.record_metric(
                    "vertex_ai_multimodal_compare_images_duration", duration
                )
                self.telemetry.record_metric(
                    "vertex_ai_multimodal_compare_images_count", 1
                )
            ...
            if self.telemetry and span:
                ...
                # Registrar métricas de error
                self.telemetry.record_metric(
                    "vertex_ai_multimodal_compare_images_error_count", 1
                )
    ...
    async def analyze_document(
        ...
        try:
            ...
            if self.vertex_ai_initialized:
                ...
                # Simulación para desarrollo/pruebas
                logger.warning(
                    "Vertex AI no inicializado. Usando análisis de documento simulado."
                )
                result = {
            ...
            if self.telemetry and span:
                ...
                # Registrar métricas
                self.telemetry.record_metric(
                    "vertex_ai_multimodal_analyze_document_duration", duration
                )
                self.telemetry.record_metric(
                    "vertex_ai_multimodal_analyze_document_count", 1
                )
            ...
            if self.telemetry and span:
                ...
                # Registrar métricas de error
                self.telemetry.record_metric(
                    "vertex_ai_multimodal_analyze_document_error_count", 1
                )
