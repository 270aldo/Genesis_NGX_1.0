...
# Optional import for Google Vision
try:
    from google.cloud import vision
    VISION_AVAILABLE = True
except ImportError:
    VISION_AVAILABLE = False
    logger.warning("Google Cloud Vision not available. OCR features will be disabled.")
...
try:
    ...
    CV2_AVAILABLE = False
    logger.warning("OpenCV (cv2) not installed. Some advanced vision features will be unavailable.")
    ...
    async def _perform_advanced_ocr(
        ...
        try:
            ...
            if not VISION_AVAILABLE:
                logger.warning("Google Vision not available for OCR")
                return {
                    "text": "",
                    "confidence": 0.0,
                    "structured_data": {},
