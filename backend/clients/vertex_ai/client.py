...
def get_env_int(var_name: str, default_value: int) -> int:
    ...
    try:
        ...
        logger.warning(
            f"Valor inválido para la variable de entorno {var_name}: '{val_str}'. Usando el valor por defecto: {default_value}"
        )
        return default_value
...
try:
    ...
    logger.warning(
        "No se pudieron importar las bibliotecas de Vertex AI. Usando modo mock."
    )
...
class VertexAIClient:
    ...
    def __init__(
        ...
        self.connection_pool = ConnectionPool(
            ...
            init_size=2, 
            ttl=600,  # 10 minutos
            project=os.getenv("GCP_PROJECT_ID", "agentes-ngx"),
        ...
        async with self._init_lock:
            ...
            try:
                ...
                telemetry_adapter.record_metric(
                    "vertex_ai.client.initializations", 1, {"status": "success"}
                )
                ...
                telemetry_adapter.record_metric(
                    "vertex_ai.client.initializations", 1, {"status": "failure"}
                )
                logger.error(
                    f"Error durante la inicialización del cliente VertexAI: {e}"
                )
                return False
            finally:
    ...
    async def _call_generate_content_api(
        self,
        model,
        prompt: str,
        ...
        """
        Llama a la API de Vertex AI con circuit breaker.
    ...
    async def _call_embedding_api(self, model, texts: List[str]) -> List:
        """
        Llama a la API de embeddings con circuit breaker.
        ...
        Returns:
            Lista de embeddings
        """
        return await asyncio.to_thread(model.get_embeddings, texts)

    def _get_cache_key(
        self, data: Any, operation: str = None, namespace: str = None
    ) -> str:
        """
    ...
    async def generate_content(
        ...
        if max_output_tokens is not None:
            span_attributes["client.max_output_tokens"] = max_output_tokens
        if top_p is not None:
            span_attributes["client.top_p"] = top_p
        if top_k is not None:
            span_attributes["client.top_k"] = top_k

        span = telemetry_adapter.start_span(
            "VertexAIClient.generate_content", attributes=span_attributes
        )
        ...
        try:
            ...
            cache_key = self._get_cache_key(
                data=cache_data, operation="generate_content", namespace=cache_namespace
            )
            ...
            if not skip_cache:
                ...
                if cached_response:
                    self.stats["latency_ms"].setdefault(
                        "content_generation", []
                    ).append(
                        0
                    )  # 0ms desde caché
                    ...
                    telemetry_adapter.set_span_attribute(
                        span, "cache.operation", "generate_content"
                    )
                    telemetry_adapter.set_span_attribute(
                        span, "cache.namespace", cache_namespace or "default"
                    )
                    telemetry_adapter.set_span_attribute(span, "client.cache", "hit")
                    telemetry_adapter.record_metric(
                        "vertex_ai.client.cache_hits", 1, {"operation": "content"}
                    )
            ...
            telemetry_adapter.record_metric(
                "vertex_ai.client.cache_misses", 1, {"operation": "content"}
            )
            ...
            try:
                ...
                if client.get("mock", False):
                    ...
                    # Generar contenido con circuit breaker
                    model = client["text_model"]

                    try:
                        result = await self._call_generate_content_api(
                            model, prompt, generation_config, system_instruction
                        )
                    except CircuitBreakerOpenError:
                        # El circuit breaker está abierto, retornar error específico
                        logger.error("Circuit breaker abierto para Vertex AI")
                        telemetry_adapter.add_span_event(span, "circuit_breaker_open")
                        raise
                    ...
                    response = {
                        "text": result.text,
                        "finish_reason": (
                            result.candidates[0].finish_reason.name
            ...
            telemetry_adapter.set_span_attribute(
                span, "client.tokens.total", response["usage"]["total_tokens"]
            )
            telemetry_adapter.record_metric(
                "vertex_ai.client.latency",
                latency_ms,
                {"operation": "content_generation"},
            )
            telemetry_adapter.record_metric(
                "vertex_ai.client.tokens",
            ...
            telemetry_adapter.record_metric(
                ...
                1,
                {"operation": "content_generation", "error_type": error_type},
            )
    ...
    async def generate_content_stream(
        ...
        """
        Genera contenido de texto usando streaming para respuestas en tiempo real.
        ...
        Yields:
            Dict[str, Any]: Chunks de respuesta con texto parcial
        """
        await self._ensure_initialized()
        
        span = telemetry_adapter.start_span(
            "VertexAIClient.generate_content_stream",
            {"client.prompt_length": len(prompt), "client.streaming": True}
        )
        ...
        try:
            if VERTEX_AI_AVAILABLE:
                ...
                contents = []
                if system_instruction:
                    contents.append({"role": "model", "parts": [{"text": system_instruction}]})
                ...
                # Yield chunks a medida que llegan
                for chunk in response_stream:
                    if chunk.text:
                        ...
                        total_tokens += len(chunk.text.split())  # Aproximación
                ...
                yield {
                    ...
                    "finish_reason": "STOP",
                    "usage": {
                        "prompt_tokens": len(prompt.split()),  # Aproximación
                ...
                # Simulación de streaming cuando Vertex AI no está disponible
                mock_response = f"Respuesta simulada en streaming para: {prompt[:50]}..."
    ...
    async def generate_embedding(self, text: str) -> Dict[str, Any]:
        ...
        span = telemetry_adapter.start_span(
            "VertexAIClient.generate_embedding", {"client.text_length": len(text)}
        )
        ...
        try:
            ...
            # Verificar caché
            cache_key = self._get_cache_key({"type": "embedding", "text": text})
            ...
            if cached:
                ...
                telemetry_adapter.set_span_attribute(span, "client.cache", "hit")
                telemetry_adapter.record_metric(
                    "vertex_ai.client.cache_hits", 1, {"operation": "embedding"}
                )
                return cached
            ...
            telemetry_adapter.set_span_attribute(span, "client.cache", "miss")
            telemetry_adapter.record_metric(
                "vertex_ai.client.cache_misses", 1, {"operation": "embedding"}
            )
            ...
            telemetry_adapter.set_span_attribute(span, "client.latency_ms", latency_ms)
            telemetry_adapter.set_span_attribute(
                span, "client.dimensions", response["dimensions"]
            )
            telemetry_adapter.record_metric(
                "vertex_ai.client.latency", latency_ms, {"operation": "embedding"}
            )
            ...
            telemetry_adapter.record_metric(
                "vertex_ai.client.errors",
                1,
                {"operation": "embedding", "error_type": error_type},
            )
    ...
    async def generate_with_functions(
        ...
        """
        Genera contenido con function calling y grounding opcional.
        
        Args:
            prompt: Prompt para el modelo
            functions: Lista de declaraciones de funciones disponibles
            system_instruction: Instrucción de sistema
            temperature: Temperatura para la generación
            grounding_config: Configuración de grounding (búsqueda web, etc.)
            
        Returns:
            Dict con la respuesta y posibles llamadas a funciones
        """
        await self._ensure_initialized()
        
        span = telemetry_adapter.start_span(
            "VertexAIClient.generate_with_functions",
            {
                "client.prompt_length": len(prompt),
                "client.functions_count": len(functions),
                "client.grounding_enabled": bool(grounding_config),
            }
        )
        
        try:
            if VERTEX_AI_AVAILABLE:
                # Convertir funciones al formato de Vertex AI
                ...
                # Crear herramienta con las funciones
                ...
                # Aplicar grounding si está configurado
                if grounding_config:
                    # Configurar búsqueda web o base de conocimiento
                    if grounding_config.get("enable_web_search"):
                        # Habilitar búsqueda web para respuestas fundamentadas
                ...
                if hasattr(response, "candidates") and response.candidates:
                    ...
                    # Extraer metadatos de grounding si existen
                    if hasattr(candidate, "grounding_metadata"):
                        result["grounding_metadata"] = {
                            "search_queries": getattr(candidate.grounding_metadata, "search_queries", []),
                            "web_search_results": getattr(candidate.grounding_metadata, "web_search_results", []),
                        }
                
                return result
                
            else:
                # Simulación cuando Vertex AI no está disponible
                return {
                    "text": f"Simulación: Respuesta para '{prompt[:50]}...' con {len(functions)} funciones disponibles",
                    "function_calls": [
                        {
                            "name": functions[0]["name"] if functions else "test_function",
                            "args": {"test_param": "test_value"},
                        }
                    ] if functions else [],
                    "grounding_metadata": {
                        "simulated": True,
                        "web_search_enabled": grounding_config.get("enable_web_search", False) if grounding_config else False,
                    },
                }
                
        except Exception as e:
            logger.error(f"Error en function calling: {str(e)}")
            telemetry_adapter.record_exception(span, e)
            return {
                "text": f"Error: {str(e)}",
                "function_calls": [],
    ...
    async def batch_embeddings(self, texts: List[str]) -> Dict[str, Any]:
        ...
        span = telemetry_adapter.start_span(
            "VertexAIClient.batch_embeddings",
            {
        ...
        try:
            ...
            telemetry_adapter.record_metric(
                "vertex_ai.client.latency", latency_ms, {"operation": "batch_embedding"}
            )
            telemetry_adapter.record_metric(
                "vertex_ai.client.batch_size", len(texts), {"operation": "embedding"}
            )
            ...
            return {"error": str(e), "embeddings": [], "dimensions": 0, "count": 0}
    ...
    async def process_multimodal(
        ...
        span = telemetry_adapter.start_span(
            "VertexAIClient.process_multimodal",
            {
                "client.prompt_length": len(prompt),
        ...
        try:
            ...
            if is_base64:
                ...
                if cached:
                    ...
                    telemetry_adapter.set_span_attribute(span, "client.cache", "hit")
                    telemetry_adapter.record_metric(
                        "vertex_ai.client.cache_hits", 1, {"operation": "multimodal"}
                    )
                    return cached
            ...
            if cache_key:
                ...
                telemetry_adapter.set_span_attribute(span, "client.cache", "miss")
                telemetry_adapter.record_metric(
                    "vertex_ai.client.cache_misses", 1, {"operation": "multimodal"}
                )
            ...
            try:
                ...
                if client.get("mock", False):
                    ...
                    result = model.generate_content(
                        [prompt, image_part], generation_config=generation_config
                    )
            ...
            telemetry_adapter.record_metric(
                "vertex_ai.client.latency", latency_ms, {"operation": "multimodal"}
            )
            telemetry_adapter.record_metric(
                "vertex_ai.client.tokens",
                response["usage"]["total_tokens"],
                {"type": "total", "operation": "multimodal"},
            )
            ...
            telemetry_adapter.record_metric(
                "vertex_ai.client.errors",
                1,
                {"operation": "multimodal", "error_type": error_type},
            )
    ...
    async def process_document(
        ...
        span = telemetry_adapter.start_span(
            "VertexAIClient.process_document",
            {
        ...
        try:
            ...
            if cached:
                ...
                telemetry_adapter.set_span_attribute(span, "client.cache", "hit")
                telemetry_adapter.record_metric(
                    "vertex_ai.client.cache_hits", 1, {"operation": "document"}
                )
                return cached
            ...
            telemetry_adapter.set_span_attribute(span, "client.cache", "miss")
            telemetry_adapter.record_metric(
                "vertex_ai.client.cache_misses", 1, {"operation": "document"}
            )
            ...
            telemetry_adapter.set_span_attribute(span, "client.latency_ms", latency_ms)
            telemetry_adapter.set_span_attribute(
                span, "client.document.pages", response["pages"]
            )
            telemetry_adapter.set_span_attribute(
                span, "client.document.entities", len(response["entities"])
            )
            telemetry_adapter.record_metric(
                "vertex_ai.client.latency", latency_ms, {"operation": "document"}
            )
            ...
            telemetry_adapter.record_metric(
                "vertex_ai.client.errors",
                1,
                {"operation": "document", "error_type": error_type},
            )

            logger.error(f"Error al procesar documento: {str(e)}")

            return {"text": "", "pages": 0, "entities": [], "error": str(e)}
