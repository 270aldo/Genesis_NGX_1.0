...
try:
    ...
    logger.warning(
        "No se pudieron importar las bibliotecas de Vertex AI para ConnectionPool. Usando modo mock."
    )
...
class ConnectionPool:
    ...
    def __init__(
        self, max_size=10, init_size=2, ttl=300, project=None, location="us-central1"
    ):
        """
        ...
        self.init_size = min(
            init_size, max_size
        )  # init_size no puede ser mayor que max_size
        self.ttl = ttl
        ...
        self._initializing_lock = (
            asyncio.Lock()
        )  # Para evitar inicializaciones concurrentes
    ...
    async def initialize(self):
        ...
        async with self._initializing_lock:  # Prevenir inicialización múltiple
            ...
            logger.info(
                f"Inicializando pool de conexiones Vertex AI con hasta {self.init_size} conexiones."
            )
            ...
            if VERTEX_AI_AVAILABLE and self.project_id:
                try:
                    ...
                    logger.info(
                        f"Vertex AI SDK inicializado para proyecto {self.project_id} y locación {self.location}"
                    )
                except Exception as e:
                    logger.error(
                        f"Error al inicializar Vertex AI SDK: {e}. El pool podría no funcionar."
                    )
                ...
                try:
                    ...
                    if not client.get("mock"):
                        ...
                        if not VERTEX_AI_AVAILABLE:
                            logger.warning(
                                "Vertex AI no disponible, deteniendo inicialización del pool."
                            )
                            break
                except Exception as e:
            ...
            logger.info(
                f"Pool de {len(self.pool)}/{self.init_size} conexiones Vertex AI inicializado."
            )
    ...
    async def _create_new_client(self):
        ...
        if not self.project_id:
            ...
            if not self.project_id:
                return {
                    "mock": True,
                    "reason": "Project ID no disponible",
                    "error": "No se pudo obtener Google Cloud Project ID",
                }
        ...
        async with self.lock:
            for entry in self.pool:
                if not entry["in_use"]:
                    if time.time() - entry["timestamp"] > self.ttl:
                        logger.info(
                            f"Cliente en pool expirado. Creando nuevo. (TTL: {self.ttl}s)"
                        )
                        try:
                            ...
                            if not entry["client"].get("mock"):
                                ...
                                self.stats["acquired"] -= 1
                                self.stats["current_in_use"] = (
                                    self.max_size - self.semaphore._value
                                )
                                return entry["client"]
                        except Exception as e:
                    ...
                    if not entry["client"].get("mock"):
                        entry["in_use"] = True
                        self.stats["reused"] += 1
                        self.stats["current_available_in_pool"] = len(
                            [e for e in self.pool if not e["in_use"]]
                        )
                        return entry["client"]
                    else:
                        self.semaphore.release()
            ...
            if len(self.pool) < self.max_size:
                try:
                    ...
                    logger.error(
                        f"Error crítico creando nuevo cliente bajo demanda: {e}"
                    )
                    self.semaphore.release()
                    self.stats["acquired"] -= 1
                    self.stats["current_in_use"] = self.max_size - self.semaphore._value
                ...
                logger.error(
                    "Pool lleno y semáforo permitió adquirir, situación inesperada. Liberando semáforo."
                )
                self.semaphore.release()
                self.stats["acquired"] -= 1
                self.stats["current_in_use"] = self.max_size - self.semaphore._value
                return {
                    "mock": True,
                    "error": "Pool Lleno Inesperadamente",
                    "reason": "Situación de carrera potencial",
                }
