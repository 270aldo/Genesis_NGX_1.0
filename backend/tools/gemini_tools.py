...
class GeminiChatInput(BaseModel):
    ...
    context: Optional[str] = Field(
        None, description="Contexto adicional para la generación"
    )
    history: Optional[List[Dict[str, str]]] = Field(
        None, description="Historial de conversación"
    )
...
class GeminiChatOutput(BaseModel):
    ...
    usage: Dict[str, int] = Field(
        default_factory=dict, description="Información de uso de tokens"
    )
...
class GeminiChatSkill(Skill):
    ...
    async def execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if history:
            ...
            text = await vertex_ai_client.chat(
                history + [{"role": "user", "content": full_prompt}], temperature
            )
        else:
            # Usar generación simple
            text = await vertex_ai_client.generate_content(
                full_prompt, temperature=temperature, max_output_tokens=max_tokens
            )
...
class GeminiSummarizeInput(BaseModel):
    ...
    max_words: int = Field(
        100, description="Longitud máxima aproximada del resumen en palabras"
    )
...
class GeminiSummarizeSkill(Skill):
    ...
    async def execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Ejecutar generación
        summary = await vertex_ai_client.generate_content(
            prompt, temperature=0.3, max_output_tokens=max_words * 5
        )
...
class GeminiAnalyzeIntentInput(BaseModel):
    ...
    categories: Optional[List[str]] = Field(
        None, description="Categorías para clasificar"
    )
...
class GeminiAnalyzeIntentOutput(BaseModel):
    ...
    categories: Dict[str, float] = Field(
        default_factory=dict, description="Puntuaciones por categoría"
    )
...
class GeminiAnalyzeIntentSkill(Skill):
    ...
    async def execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Ejecutar generación
        result_text = await vertex_ai_client.generate_content(prompt, temperature=0.1)
...
class GeminiTranslateInput(BaseModel):
    ...
    target_language: str = Field(
        ..., description="Idioma destino (código ISO o nombre del idioma)"
    )
    source_language: Optional[str] = Field(
        None,
        description="Idioma origen (si se deja vacío, se detectará automáticamente)",
    )
    preserve_formatting: bool = Field(
        True, description="Mantener el formato del texto original"
    )
...
class GeminiTranslateOutput(BaseModel):
    ...
    detected_source_language: Optional[str] = Field(
        None, description="Idioma origen detectado (si no se especificó)"
    )
    character_count: int = Field(
        ..., description="Número de caracteres en el texto traducido"
    )
...
class GeminiTranslateSkill(Skill):
    ...
    async def execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Ejecutar generación
        translated_text = await vertex_ai_client.generate_content(
            prompt,
        ...
        if not source_language:
            ...
            detected_source = await vertex_ai_client.generate_content(
                detect_prompt, temperature=0.1, max_output_tokens=20
            )
            # Limpiar la respuesta (podría tener información adicional)
            detected_source = (
                detected_source.strip().split("\n")[0].replace(".", "").strip()
            )
...
class GeminiSentimentAnalysisInput(BaseModel):
    ...
    text: str = Field(..., description="Texto a analizar")
    detailed: bool = Field(
        False, description="Si es True, incluye análisis detallado de emociones"
    )
...
class GeminiSentimentAnalysisOutput(BaseModel):
    ...
    emotions: Optional[Dict[str, float]] = Field(
        None, description="Puntuaciones detalladas por emoción"
    )
...
class GeminiSentimentAnalysisSkill(Skill):
    ...
    async def execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Ejecutar análisis
        result = await vertex_ai_client.analyze_sentiment(text, detailed)
...
class GeminiImageAnalysisSkill(Skill):
    ...
    async def execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Ejecutar análisis
        analysis = await vertex_ai_client.analyze_image(image_path, prompt)
...
class GeminiPdfAnalysisSkill(Skill):
    ...
    async def execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Ejecutar análisis
        analysis = await vertex_ai_client.analyze_pdf(pdf_path, prompt, max_pages)
...
class GeminiCsvAnalysisSkill(Skill):
    ...
    async def execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Ejecutar análisis
        analysis = await vertex_ai_client.analyze_csv(csv_path, prompt, sample_rows)
