...
from core.async_processor import async_processor, TaskPriority
...
class TaskSubmitRequest(BaseModel):
    ...
    priority: str = Field(
        default="MEDIUM",
        description="Prioridad de la tarea (HIGH, MEDIUM, LOW, BACKGROUND)",
    )
    timeout: Optional[float] = Field(
        default=None, description="Tiempo máximo de ejecución en segundos"
    )
    max_retries: Optional[int] = Field(
        default=None, description="Número máximo de reintentos"
    )
    retry_delay: Optional[float] = Field(
        default=None, description="Tiempo de espera entre reintentos en segundos"
    )
...
class ProcessorStatsResponse(BaseModel):
    ...
    queue_sizes: Dict[str, int] = Field(
        ..., description="Tamaño de las colas por prioridad"
    )
    status_counts: Dict[str, int] = Field(
        ..., description="Número de tareas por estado"
    )
    priority_counts: Dict[str, int] = Field(
        ..., description="Número de tareas por prioridad"
    )
...
async def submit_task(
    request: TaskSubmitRequest, user_id: str = Depends(get_current_user)
):
    """
    ...
    if request.function_name not in AVAILABLE_FUNCTIONS:
        raise HTTPException(
            status_code=400, detail=f"Función '{request.function_name}' no disponible"
        )
    ...
    try:
        ...
        return TaskSubmitResponse(task_id=task_id, status=task_info["status"])
...
async def cancel_task(
    ...
    if not success:
        raise HTTPException(
            status_code=400, detail=f"No se pudo cancelar la tarea {task_id}"
        )
...
async def clear_completed_tasks(
    older_than: Optional[int] = Query(
        None, description="Eliminar tareas completadas hace más de X segundos"
    ),
    user_id: str = Depends(get_current_user),
):
    """
...
async def get_available_functions(user_id: str = Depends(get_current_user)):
    """
