...
from core.batch_processor import batch_processor, BatchStrategy
...
class BatchProcessRequest(BaseModel):
    ...
    strategy: str = Field(
        default="adaptive",
        description="Estrategia de división en lotes (chunk_size, chunk_count, adaptive)",
    )
    chunk_size: Optional[int] = Field(
        default=None, description="Tamaño de cada lote (para chunk_size)"
    )
    chunk_count: Optional[int] = Field(
        default=None, description="Número de lotes (para chunk_count)"
    )
    timeout: Optional[float] = Field(
        default=None, description="Tiempo máximo de ejecución en segundos"
    )
...
class BatchStatusResponse(BaseModel):
    ...
    successful_items: int = Field(
        ..., description="Número de elementos procesados correctamente"
    )
    ...
    execution_time: Optional[float] = Field(
        default=None, description="Tiempo de ejecución en segundos"
    )
...
class BatchResultsResponse(BaseModel):
    ...
    errors: Optional[Dict[str, str]] = Field(
        default=None, description="Errores del procesamiento"
    )
    total_items: int = Field(..., description="Número total de elementos")
    successful_items: int = Field(
        ..., description="Número de elementos procesados correctamente"
...
class ProcessorStatsResponse(BaseModel):
    ...
    partial_batches: int = Field(
        ..., description="Número de lotes completados parcialmente"
    )
    total_items_processed: int = Field(
        ..., description="Número total de elementos procesados"
    )
    successful_items: int = Field(
        ..., description="Número total de elementos procesados correctamente"
    )
...
AVAILABLE_PROCESSORS = {
    "text_sentiment": {
        "module": "clients.vertex_ai_client",
...
async def process_batch(
    request: BatchProcessRequest, user_id: str = Depends(get_current_user)
):
    """
    ...
    if request.processor_name not in AVAILABLE_PROCESSORS:
        raise HTTPException(
            status_code=400,
            detail=f"Procesador '{request.processor_name}' no disponible",
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error al iniciar procesamiento: {str(e)}"
        )
...
async def get_batch_results(
    ...
    max_results: Optional[int] = Query(
        None, description="Número máximo de resultados a devolver"
    ),
    user_id: str = Depends(get_current_user),
):
    """
    ...
    try:
        results = await batch_processor.get_batch_results(
            batch_id=batch_id, include_errors=include_errors, max_results=max_results
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error al obtener resultados: {str(e)}"
        )
...
async def cancel_batch(
    ...
    if not success:
        raise HTTPException(
            status_code=400, detail=f"No se pudo cancelar el lote {batch_id}"
        )
...
async def get_processor_stats(user_id: str = Depends(get_current_user)):
    """
...
async def clear_completed_batches(
    older_than: Optional[int] = Query(
        None, description="Eliminar lotes completados hace más de X segundos"
    ),
    user_id: str = Depends(get_current_user),
):
    """
...
async def get_available_processors(user_id: str = Depends(get_current_user)):
    """
