...
def get_orchestrator() -> NGXNexusOrchestrator:
    ...
    if _orchestrator_instance is None:
        ...
        logger.debug(
            f"NGXNexusOrchestrator se inicializará con a2a_server_url (para HTTP A2A) = {http_a2a_target_url}"
        )
...
async def chat(
    ...
    try:
        ...
        logger.info(
            f"Procesando mensaje de chat para usuario {effective_user_id}, sesión {session_id}"
        )

        # Preparar contexto
        context = request.context or {}
        context.update({"user_id": effective_user_id, "session_id": session_id})
        ...
        # 🚀 INTEGRACIÓN CRM: Tracking de uso de agentes
        try:
            from app.services.crm_integration import track_agent_usage
            import time
            
            # Calcular tokens aproximados (estimación basada en caracteres)
            estimated_tokens = max(1, len(request.text + response_text) // 4)
            
            # Calcular tiempo de respuesta
            response_time_ms = int(metadata.get("processing_time_ms", 0))
            
            # Obtener tier de suscripción del contexto o metadata
            subscription_tier = context.get("subscription_tier") or metadata.get("subscription_tier", "essential")
            organization_id = context.get("organization_id") or metadata.get("organization_id")
            
            # Enviar evento de uso para cada agente usado
            for agent_id in agents_used:
                background_tasks.add_task(
                    ...
                    context={
                        ...
                        "confidence": max([ar.confidence for ar in agent_responses], default=1.0),
                        "agents_count": len(agents_used)
                    }
                )
                
            logger.debug(f"Enviando tracking de uso para agentes {agents_used} al CRM")
            
        except Exception as tracking_error:
            # No fallar la respuesta si el tracking falla
            logger.warning(f"Error en tracking de uso para CRM: {tracking_error}")

        logger.info(
            f"Mensaje de chat procesado correctamente para usuario {effective_user_id}"
        )
