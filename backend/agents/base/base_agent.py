...
# Importar capabilities de colaboración
from agents.skills.collaboration_skills import CollaborationMixin

# Configurar logging
logger = logging.getLogger(__name__)


class BaseAgent(GoogleAgent, CollaborationMixin):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Inicializar CollaborationMixin
        CollaborationMixin.__init__(self)
        ...
        self.model = kwargs.get(
            "model", None
        )  # Asignar un valor por defecto si no está
        self.instruction = kwargs.get(
            "instruction", None
        )  # Asignar un valor por defecto si no está
    ...
    async def run(
        ...
        **kwargs,
        ...
        logger.info(
            f"[{execution_id}] Ejecutando agente {self.agent_id} para usuario {user_id or 'anónimo'}"
        )
        logger.debug(
            f"[{execution_id}] Input: {user_input[:100]}{'...' if len(user_input) > 100 else ''}"
        )
        ...
        try:
            ...
            execution_time = time.time() - start_time
            logger.error(
                f"[{execution_id}] Timeout al ejecutar agente para usuario {user_id} después de {execution_time:.2f}s"
            )
            return {
                "status": "error",
                "error": "timeout",
            ...
            execution_time = time.time() - start_time
            logger.error(
                f"[{execution_id}] Error al ejecutar agente: {exc}", exc_info=True
            )
            return {
                "status": "error",
                "error": str(exc),
    ...
    async def _run_async_impl(
        ...
        **kwargs,
        ...
        implementación básica que incluye análisis de colaboración inteligente.
        ...
        # Check if collaboration should be offered
        ...
        # Quick check if this might benefit from collaboration
        if hasattr(self, 'should_offer_collaboration') and self.should_offer_collaboration(user_input):
            logger.info(f"Agent {self.agent_id} detected potential collaboration opportunity")
            
            # Generate individual response first
            individual_response = await self._generate_individual_response(
                user_input, user_id, session_id, **kwargs
            )
            
            # If individual response was successful, enhance with collaboration option
            if individual_response.get("status") == "success":
                enhanced_result = await self.generate_smart_response_with_collaboration_option(
                    user_input=user_input,
                    individual_response=individual_response.get("response", ""),
                    context=context
                )
                
                # Merge results
                result = individual_response.copy()
                result.update(enhanced_result)
                return result
            else:
                # If individual response failed, just return it
                return individual_response
        else:
            # No collaboration suggested - generate normal individual response
            return await self._generate_individual_response(
                user_input, user_id, session_id, **kwargs
            )
    ...
    async def _generate_individual_response(
        ...
        **kwargs,
        ...
        """Generate individual agent response (to be overridden by subclasses)"""
        
        # Esta implementación debe ser sobreescrita por las clases derivadas
        return {
            "status": "error",
            "error": "not_implemented",
            "response": "Este agente no ha implementado el método _generate_individual_response.",
        }
    ...
    def add_to_history(self, user_input: str, agent_response: str) -> None:
        ...
        self._conversation_history.append(
            {"timestamp": time.time(), "user": user_input, "agent": agent_response}
        )
