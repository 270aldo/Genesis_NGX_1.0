...
class BaseNGXAgent(ADKAgent):
    ...
    def __init__(
        ...
        **kwargs
        ...
        instruction = kwargs.pop('instruction', f"You are {agent_name}, a specialized {agent_type} agent.")
        ...
        logger.info(f"Initialized {agent_name} ({agent_id}) with personality {personality_type}")
    ...
    async def execute_skill(self, skill_name: str, **kwargs) -> Any:
        ...
        try:
            ...
            self._skill_metadata[skill_name]["last_executed"] = datetime.now().isoformat()
    ...
    async def get_cached_response(self, cache_key: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if cached:
                ...
                return eval(cached)  # Safe in controlled environment
            else:
                self._metrics["cache_misses"] += 1
                return None
    ...
    async def restore_state(self) -> None:
        ...
        if self.redis_manager and await self.redis_manager.is_connected():
            try:
                ...
                if stored_state:
                    self._state.update(eval(stored_state))  # Safe in controlled environment
            except Exception as e:
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        if self._metrics["response_times"]:
            summary["avg_response_time"] = (
                sum(self._metrics["response_times"]) / 
    ...
    async def handle_error(
        ...
        logger.error(
            f"Error in {self.agent_id}: {str(error)}",
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.redis_manager:
                health_status["checks"]["redis"] = await self.redis_manager.is_connected()
            else:
