...
class MockMeter:
    def create_counter(self, *args, **kwargs):
        ...
        if (
            ...
            and (len(args[0]) > 63 or not args[0].isascii())
        ):
...
class ADKAgent(A2AAgent, GoogleADKAgent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.vertex_ai_client = vertex_ai_client if vertex_ai_client else VertexAIClient()
        ...
        # TODO: This needs to be called after initialization in an async context
        ...
        if (
            not hasattr(self, "skills")
            or not self.skills
            or not isinstance(self.skills, list)
        ):
            logger.warning(
                f"Agente {name} ({agent_id}) no definió self.skills. No se registrarán skills."
            )
            ...
            processed_google_adk_tools, processed_a2a_skills_for_card, skill_names = (
                self._initialize_and_prepare_skills()
            )
        ...
        GoogleADKAgent.__init__(
            self, toolkit=self.adk_toolkit, name=name, description=description, **kwargs
        )
    ...
    async def _initialize_vision_capabilities(self):
        ...
        try:
            ...
            logger.info(
                f"Capacidades de visión y multimodales inicializadas para el agente '{self.name}'"
            )
        except Exception as e:
            logger.error(
                f"Error al inicializar capacidades de visión para el agente '{self.name}': {e}",
                exc_info=True,
            )
    ...
    def _setup_telemetry(self, service_name: str):
        ...
        otel_req_counter_name = self._sanitize_otel_name(
            f"{service_name.replace(' ', '_')}_req_total"
        )
        otel_resp_time_hist_name = self._sanitize_otel_name(
            f"{service_name.replace(' ', '_')}_resp_time_secs"
        )

        if not has_telemetry:
            logger.warning(
                "OpenTelemetry SDK no instalado. La telemetría detallada está deshabilitada. Usando mocks."
            )
        ...
        try:
            resource = Resource(
                attributes={ResourceAttributes.SERVICE_NAME: otel_service_name}
            )
            ...
            self.request_counter = self.meter.create_counter(
                otel_req_counter_name, "1", "Total requests"
            )
            self.response_time_histogram = self.meter.create_histogram(
                otel_resp_time_hist_name, "s", "Response time"
            )

            logger.info(
                f"Telemetría configurada para el servicio '{otel_service_name}'."
            )

        except Exception as e:
            logger.error(
                f"Error en la configuración de telemetría para '{otel_service_name}': {e}. Usando mocks.",
                exc_info=True,
            )
            ...
            self.response_time_histogram = self.meter.create_histogram(
                "mock_resp_time_fallback"
            )

    def _initialize_and_prepare_skills(
        self,
    ) -> Tuple[List[Callable], List[Dict[str, Any]], List[str]]:
        processed_google_adk_tools = []
        processed_a2a_skills_for_card = []
        skill_names_for_card = (
            []
        )  # Nombres para las A2A cards, que podrían ser diferentes
        ...
        for skill_object in self.skills:  # self.skills es ahora List[Skill]
            ...
            try:
                with self.tracer.start_as_current_span(f"register_skill_{skill_name}"):
                    if hasattr(self.adk_toolkit, "register_skill"):
                        ...
                        logger.warning(
                            f"No se pudo registrar la skill '{skill_name}' en el toolkit de Google ADK."
                        )
            except Exception as e:
                logger.error(
                    f"Error registrando skill '{skill_name}': {e}", exc_info=True
                )
            ...
            if skill_callable and callable(skill_callable):
                ...
                logger.warning(
                    f"Skill '{skill_name}' no tiene un atributo 'handler' que sea callable. Saltando para Google ADK tools."
                )
            ...
            if input_schema_pydantic:
                try:
                    ...
                    if not (
                        isinstance(input_schema_pydantic, type)
                        and issubclass(input_schema_pydantic, BaseModel)
                    ):
                        raise TypeError(
                            f"input_schema para {skill_name} no es un modelo Pydantic."
                        )
                    ...
                    logger.error(
                        f"Error generando JSON schema para input de {skill_name}: {e}"
                    )

            if output_schema_pydantic:
                try:
                    if not (
                        isinstance(output_schema_pydantic, type)
                        and issubclass(output_schema_pydantic, BaseModel)
                    ):
                        raise TypeError(
                            f"output_schema para {skill_name} no es un modelo Pydantic."
                        )
                    ...
                    logger.error(
                        f"Error generando JSON schema para output de {skill_name}: {e}"
                    )
    ...
    async def _get_context(
        ...
        """
        logger.warning(
            f"_get_context no implementado en ADKAgent base para user_id {user_id}"
        )
    ...
    async def _update_context(
        ...
        **kwargs,
        ...
        """
        logger.warning(
            f"_update_context no implementado en ADKAgent base para user_id {user_id}"
        )

    async def _get_program_type_from_profile(
        ...
        return (
            selected_program if selected_program in valid_program_types else "GENERAL"
        )
    ...
    async def _extract_profile_details(self, client_profile: Dict[str, Any]) -> str:
        ...
        if client_profile:
            ...
            if goals := client_profile.get("goals"):
                details.append(f"- Objetivos: {goals}")
            if level := client_profile.get("experience_level"):
                details.append(f"- Experiencia: {level}")
            if metrics := client_profile.get("current_metrics"):
                details.append(f"- Métricas: {metrics}")
            if prefs := client_profile.get("preferences"):
                details.append(f"- Preferencias: {prefs}")
            if injuries := client_profile.get("injury_history"):
                details.append(f"- Lesiones: {injuries}")
    ...
    async def identify_objects_in_image(self, image_data, min_confidence=0.5):
        ...
        with self.tracer.start_as_current_span("agent_identify_objects_in_image"):
            return await self.vision_processor.identify_objects(
                image_data, min_confidence
            )
    ...
    async def describe_image(
        self, image_data, detail_level="standard", focus_aspect=None
    ):
        """
        ...
        with self.tracer.start_as_current_span("agent_describe_image"):
            return await self.vision_processor.describe_image(
                image_data, detail_level, focus_aspect
            )

    # Métodos para capacidades multimodales
    async def process_multimodal(
        self, prompt, image_data, temperature=0.7, max_output_tokens=None
    ):
        """
        ...
        with self.tracer.start_as_current_span("agent_process_multimodal"):
            return await self.multimodal_adapter.process_multimodal(
                prompt, image_data, temperature, max_output_tokens
            )

    async def visual_qa(
        self, question, image_data, temperature=0.2, max_output_tokens=1024
    ):
        """
        ...
        with self.tracer.start_as_current_span("agent_visual_qa"):
            return await self.multimodal_adapter.visual_qa(
                question, image_data, temperature, max_output_tokens
            )

    async def generate_image_caption(
        self, image_data, style="descriptive", max_length=None
    ):
        """
        ...
        with self.tracer.start_as_current_span("agent_generate_image_caption"):
            return await self.multimodal_adapter.generate_image_caption(
                image_data, style, max_length
            )
    ...
    async def compare_images(self, image_data1, image_data2, comparison_aspects=None):
        ...
        with self.tracer.start_as_current_span("agent_compare_images"):
            return await self.multimodal_adapter.compare_images(
                image_data1, image_data2, comparison_aspects
            )
    ...
    async def run(self, *args, **kwargs):
        ...
        try:
            ...
            if self.vertex_ai_client and hasattr(self.vertex_ai_client, 'set_current_agent'):
                self.vertex_ai_client.set_current_agent(self.agent_id)
            ...
            if budget_status.get("percentage", 0) > 90:
                logger.warning(
                    f"Agente {self.agent_id} está cerca del límite de presupuesto: {budget_status.get('percentage')}%"
                )
